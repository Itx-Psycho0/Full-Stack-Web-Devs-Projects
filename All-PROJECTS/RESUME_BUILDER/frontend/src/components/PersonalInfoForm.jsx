import { BriefcaseBusiness, User, Mail, Phone, MapPin, Globe, Link } from 'lucide-react'
...
const PersonalInfoForm = ({ data, onChange, removeBackground, setRemoveBackground }) => {
    ...
    const fields = [
        ...
        { key: 'linkedin', label: 'LinkedIn', icon: Link, type: 'url', required: false },
        { key: 'portfolio', label: 'Portfolio', icon: Globe, type: 'url', required: false },
        ...
        <div>
            ...
            <div className='flex items-center gap-2'>
                <label>
                    ...
                    <input type='file' accept='image/jpeg, image/png' className='hidden' onChange={(e) => handleImageChange('image', e.target.files[0])} />
                </label>
                {typeof data.image === 'object' && (
                    <div className='flex flex-col gap-1 pl-4 text-sm'>
                        <p>Remove Background</p>
                        <label className='relative inline-flex items-center cursor-pointer text-gray-900 gap-3'>
                            <input type='checkbox' onChange={() => setRemoveBackground(prev => !prev)} checked={removeBackground} className='sr-only peer' />
                            ...
                            <span className='text-sm font-medium text-gray-900'>Remove Background</span>
                        </label>
                    </div>
                )}
